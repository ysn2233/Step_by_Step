"Usage: reorganiser.py [-n|d] [-l|h] <path to source file>"
import sys
import ast
import cStringIO
import os
...
class Reorganiser:
    ...
    generate re-organised source code for the abstract syntax."""

    def __init__(self, tree, mode=settings.NORMAL, level=settings.LOW):
        "Initialise re-organiser."
        self.front_code = []
        self.back_code = []
        self.tree = tree
        self.future_imports = []
    ...
    def run(self):
        ...
        self.search()
        self.flush()
        return self.front_code, self.back_code
    ...
    def newline(self):
        ...
        if self.curr_node == "":
            if not self.comp_stat:
                ...
                self.dep_graph.add_node(self.curr_node)
                self.buf_list[self.curr_node] = self.curr_buf
                self.curr_buf = cStringIO.StringIO()
                self.stat_list.append(self.curr_node)
                for i in range(len(self.call_list)):
                    self.dep_graph.add_edge(self.curr_node, self.call_list[i], weight=i)
    ...
    def flush(self):
        for node in self.out_list:
            ...
            if re.match(r"(_stat_[0-9]+_)", node):
                self.front_code.extend(self.buf_list[node].getvalue().split("\n")[:-1])
                self.back_code.extend(self.buf_list[node].getvalue().split("\n")[:-1])
                continue
            if self.level == settings.LOW:
                self.front_code.extend(self.buf_list[node].getvalue().split("\n")[:-1])
            else:
                self.front_code.append(self.buf_list[node].getvalue()[:-1])
            def_code = self.buf_list[node].getvalue().split("\n")[:-1]
            self.back_code.append(def_code[0])
            if self.docs_list.has_key(node):
                self.back_code.extend(self.docs_list[node].split("\n")[:-1])
            self.back_code.extend(def_code[1:])
    ...
    def _FunctionDef(self, t):
        ...
        if (isinstance(t.body[0], ast.Expr) and
            ...
            for i in range(len(docs_lines)):
                self.docs_list[t.name] += "    " * self.indents
                if i == 0:
                    self.docs_list[t.name] += "\"\"\""
...
def roundtrip(filename, output=sys.stdout, mode=settings.NORMAL, level=settings.LOW):
    ...
    front_code, back_code = Reorganiser(tree, mode, level).run()
    for l in back_code:
        output.write(l)
        output.write("\n")
